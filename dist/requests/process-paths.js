...
function processPaths(pathsWithParameters, swaggerPath, config, definitions, basePath) {
    ...
    _.forEach(controllerFiles, (methods, name) => (0, process_controller_1.processController)(methods, name, config, definitions));
}
    ...
    Object.values(paths).forEach(pathValue => {
        if (pathValue.parameters) {
            Object.keys(pathValue).forEach(key => {
                ...
                method.parameters = (0, utils_1.merge)(method.parameters, pathValue.parameters, 'in', 'name');
            });
        }
