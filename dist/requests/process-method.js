...
function processMethod(method, unwrapSingleParamMethods) {
    let methodDef = '';
    let interfaceDef = '';
    const url = method.url.replace(/{([^}]+)}/g, (_, key) => `\${${(0, common_1.getAccessor)(key, 'pathParams')}}`);
    ...
    if (method.paramDef) {
        ...
        paramGroups = (0, lodash_1.groupBy)(paramDef, 'in');
        const paramsType = (0, lodash_1.upperFirst)(`${method.simpleName}Params`);
    ...
    methodDef += '\n';
    methodDef += (0, utils_1.makeComment)([method.summary, method.description, method.swaggerUrl].filter(Boolean));
...
function getParamSeparation(paramGroups) {
    return (0, lodash_1.map)(paramGroups, (group, groupName) => {
        let baseDef;
        let def;
        const list = (0, lodash_1.map)(group, p => {
            ...
            if (groupName === 'header' && p.type !== 'string')
                ...
                suffix = '';
            return (0, common_1.getObjectPropSetter)(p.name, 'params', suffix);
        });
        if (groupName === 'query') {
