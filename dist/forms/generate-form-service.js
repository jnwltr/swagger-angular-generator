...
function generateFormService(config, name, params, definitions, simpleName, formSubDirName, className) {
    let content = '';
    const formName = `${_.lowerFirst(name)}Form`;
...
function getFormSubmitFunction(name, formName, simpleName, paramGroups) {
    let res = utils_1.indent(`${_.lowerFirst(name)}() {\n`);
    res += utils_1.indent(`this.${_.lowerFirst(name)}Service.${simpleName}(${getSubmitFnParameters(formName, paramGroups)});\n`, 2);
    res += utils_1.indent('}\n');
    return res;
}
