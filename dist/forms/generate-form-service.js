...
function makeField(param, name, required, definitions, parentTypes) {
    ...
    if (!param.type) {
        ...
        const defType = (0, common_1.normalizeDef)(refType);
    ...
    if (type === 'array') {
        ...
        if (!controlInstance)
            return;
        initializer += `(\n${(0, utils_1.indent)(controlInstance)}), []`;
    }
    else if (type === 'object') {
        const def = {
            name: '',
    ...
    if (name) {
        name = (0, common_1.getAccessor)(name);
        res = `${name}: ${res},`;
    }
    return res;
}
...
function getFormSubmitFunction(name, formName, simpleName, paramGroups) {
    ...
    res += (0, utils_1.indent)(`return this.${_.lowerFirst(name)}Service.${simpleName}(${params});\n`, 2);
    res += (0, utils_1.indent)('}\n');
    return res;
}
