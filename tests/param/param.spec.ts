...
import {DashedParams, ParamsService} from '../../demo/generated/controllers/Params';
import {DashedFormService} from '../../demo/generated/store/params/dashed/dashed.service';
...
describe(`Param call`, () => {
  ...
  it(`should set and send dashed params properly`, () => {
    ...
    expect(service.form.value !== mock).withContext('Form object does not differ from mock object').toBeTruthy();
    expect(service.form.value).withContext('Form value differs from mock value').toEqual(mock);
    ...
    // query params are always strings, so we need to map them to strings
    expect(req.params.getAll('queryParamCollectionMulti').map(String))
      .toEqual(mock.queryParamCollectionMulti.map(String));
